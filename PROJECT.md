...
1. **Early Persistent File Logger (`src/proxy/early_logger.h`, `early_logger.cpp`)**:
   ...
   - Thread-safe via `SRWLOCK` or `CRITICAL_SECTION`. Pure Win32 Kernel32 APIs (zero CRT buffer dependencies).
...
5. **Hardware & E2E Verification (`tests/`, `build.bat`, `nvp_live_test.exe`)**:
...
## Feature Inventory
Every feature from user requirements is mapped to an implementation milestone.
...
| F7 | ResizeBuffers Interception | Hooks `IDXGISwapChain::ResizeBuffers` (slot 13), invalidates cached state | M1 | ORIGINAL_REQUEST §R4 |
| F8 | ResizeBuffers1 Interception | Hooks `IDXGISwapChain::ResizeBuffers1` (slot 39), invalidates cached state | M1 | ORIGINAL_REQUEST §R4 |
| F9 | State Re-establishment | Re-establishes wrapper handles and bridge state smoothly on subsequent Present | M1 | ORIGINAL_REQUEST §R4 |
| F10 | Output Interface Caching | Caches `IDXGIOutput` via `IDXGISwapChain::GetContainingOutput` | M1 | ORIGINAL_REQUEST §R3 |
| F11 | VBlank Pacing for Dual Presents | Invokes `output->WaitForVBlank()` between Present 1 and Present 2 | M1 | ORIGINAL_REQUEST §R3 |
| F12 | VBlank Fallback Handling | Handles windowed, minimized, and `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE` | M1 | ORIGINAL_REQUEST §R3 |
| F13 | Complete Clean MSVC C++17 Build | Zero errors via `build.bat` producing all targets | M2 | ORIGINAL_REQUEST §R5 |
| F14 | Local RTX 3080 Hardware Verification | Live test execution via `nvp_live_test.exe` verifying CUDA graphs & frame gen | M2 | ORIGINAL_REQUEST §R5 |

## Milestones
| # | Name | Scope | Dependencies | Status |
|---|------|-------|-------------|--------|
| 1 | M1: Proxy Runtime Hardening | F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12 | none | IN_PROGRESS |
| 2 | M2: E2E Verification & Hardware Regression | F13, F14 | M1 | PLANNED |
...
### Safe Wrapper Probe Contract (`src/proxy/sm86_rehost.cpp`)
```cpp
...
### ResizeBuffers Lifecycle Contract (`src/proxy/sm86_rehost.cpp`)
```cpp
...
## Code Layout
- `src/proxy/early_logger.h`: Persistent early file logging interface.
- `src/proxy/sm86_rehost.cpp`: Proxy entry, DllMain, DXGI hooks, wrapper inspection, ResizeBuffers hooks.
- `src/proxy/d3d11_to_d3d12_bridge.cpp`: D3D11 shadow swapchain bridge with safe wrapper inspection.
- `src/proxy/proxy.cpp`: Dual-present orchestration and VBlank pacing.
- `tests/test_proxy_hardening.cpp`: Automated verification harness for safety, logging, resize, and pacing.
